---
name: pdm-design-doc
description: PdMとしてDesign Docを作成・仕様書に変換するワークフロースキル。「Design Docを作りたい」「機能の仕様を整理したい」「仕様書を書きたい」「何を作るか整理したい」「アイデアをDesign Docにまとめたい」など、プロダクト開発の上流設計が絡む作業では必ずこのスキルを参照すること。アイデア段階の曖昧な入力からでも始められる。PdM-Playbook の Skill 2-3（議論→Design Doc / Design Doc→仕様書）をカバー。前段の「ユーザーの声→問題定義」は pdm-voice-to-problem、後段の「仕様書→実装プロンプト」は pdm-spec-to-prompt を使う。
---

# PdM Design Doc 作成スキル

アイデア・問題の断片から出発し、**Design Doc → 仕様書** まで一貫して作るワークフロー。

> **PdM-Playbook 上の位置**: Skill 2（議論→Design Doc）と Skill 3（Design Doc→仕様書）を担当。
> - **前段**: ユーザーの声を整理したいときは `pdm-voice-to-problem`（Skill 1）
> - **後段**: 仕様書を AI 実装プロンプトに変換したいときは `pdm-spec-to-prompt`（Skill 4）
> - **取捨選択**: 複数機能の優先順位は `pdm-priority-matrix`（Skill 5）、リリーススコープ管理は `pdm-scope-management`（Skill 6）

ユーザーがどこから始めるかは毎回異なる。状況を見て適切なフェーズに飛び込む：

| ユーザーの状態 | 始めるフェーズ |
|---|---|
| 「〇〇を作りたい」など曖昧なアイデア | Phase 1（ディスカッション） |
| 問題はある程度整理されている | Phase 2（Design Docドラフト） |
| Design Docはある、仕様書が欲しい | Phase 3（仕様書変換） |
| ユーザーの声がまだ整理されていない | **`pdm-voice-to-problem` を先に呼ぶ** |
